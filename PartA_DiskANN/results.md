...
The measured build is dominated by Python work in `_robust_prune`. These runs
do not establish the build cost of a future C++ DuckDB extension or whether
1M-scale builds would be CPU- or memory-bound. The chunked builder in the
proposal targets memory use by avoiding holding all neighbor lists at once.
...
- The proposal's recall@10 ≥ 0.95 bar is cleared on this real-data run:
  0.998 at L=64. The same hyperparameters remain below the bar on the
  Gaussian run (§1.1: 0.876 at n=10k, L=256). These two datasets differ in
  more than cluster structure, so the runs do not isolate the cause of the
  recall gap.
...
- The mmap warm-cache surcharge is 1.27-1.65×, within §2.1's observed
  Gaussian range. Build time is lower than in the Gaussian n=10k run
  (125 s vs 306 s); one run per dataset does not identify the cause.
